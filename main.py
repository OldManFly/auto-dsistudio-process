...
DSI_path = './dsi_studio/Contents/MacOS/dsi_studio'
...
#def button_mask():
#	global mask_select
#	mask_select = filedialog.askdirectory()
#	mask_show.set(mask_select)
...
#	labelText.set(mode.get())   ##teat
	...
	for i in subject_list:
		src(i,result+'/src/'+i.split("/")[-4]+'.src.gz')
...
#		total = len(fib_file)
		for j in fib_file:
			roi_based(j,atlas_select,all_roi,result+'/ROI_index/'+j.split('/')[-1].split('.')[0]+'.txt')
#			print('\r' + '[src Progress]:[%s%s]%.2f%%;' % ('█' * int(temp*20/total), ' ' * (20-int(temp*20/total)),float(temp/total*100)), end='')
#			sleep(0.01)
#			print('ROI now')
...
#		total =len(fib_file)
		for track in fib_file:
			 for com_roi in combinations(roi,2):
				  fiber_tracking(track,atlas_select,com_roi[-1],result+'/Tracking_index/'+com_roi[1],com_roi[0].split('/')[-1]+'_'+com_roi[1].split('/')[-1]+'_track.trk')
#				  print('\r' + '[src Progress]:[%s%s]%.2f%%;' % ('█' * int(temp*20/total), ' ' * (20-int(temp*20/total)),float(temp/total*100)), end='')
#				  sleep(0.01)
#				  print('tracking now')
...
#	mask_path = tk.Button(window, text='mask path', command=lambda:button_mask())
#	mask_path.grid(column=0,row=30)
#	
#	mask_show = tk.StringVar()  ##接button 的值
#	mask_box = tk.Label(window, textvariable=mask_show, bg='white', fg='black', font=('Arial', 12), width=30, height=2)
#	mask_box.grid(column=0,row=35)
